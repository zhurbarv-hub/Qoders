# -*- coding: utf-8 -*-
"""
SQLAlchemy модели для дедлайнов и типов дедлайнов
"""
...
class Deadline(Base):
    ...
    deadline_type_id = Column(Integer, ForeignKey('deadline_types.id'), nullable=True, index=True)  # Nullable для возможности удаления типов
    cash_register_id = Column(Integer, ForeignKey('cash_registers.id', ondelete='CASCADE'), nullable=True, index=True)
    cash_register_model = Column(String(100), nullable=True)  # Модель ККТ (дублируется из cash_register)
    ...
    # Отношения
    client = relationship("User", foreign_keys=[client_id], back_populates="client_deadlines")
    user = relationship("User", foreign_keys=[user_id], back_populates="user_deadlines")
